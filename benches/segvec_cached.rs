...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("SegVec/Linear<1024>", |b| {
        let mut v: SegVec<usize, Linear<1024>> = SegVec::with_capacity(0);
        for i in 0..N {
            v.push(i);
        }
    ...
    group.bench_function("SegVec/cached", |b| {
        let mut v: SegVec<usize> = SegVec::with_capacity(0);
        for i in 0..N {
            v.push(i);
        }
        ...
        b.iter_with_large_drop(|| {
            for i in 0..N {
                _ = black_box(v.get_cached(black_box(i), &mut cache));
            }
        });
    });
    ...
    group.bench_function("SegVec/cached", |b| {
        ...
        b.iter_with_large_drop(|| {
            for _ in 0..N {
                _ = black_box(v.get_cached(fast_prng(&mut prng_state) % N, &mut cache));
            }
        });
    });
    ...
    group.bench_function("std Vec", |b| {
        ...
        b.iter_with_large_drop(|| {
            for _ in 0..N {
                index = (index + (fast_prng(&mut prng_state) % 1000)).saturating_sub(500).min(N);
    ...
    group.bench_function("SegVec", |b| {
        ...
        b.iter_with_large_drop(|| {
            for _ in 0..N {
                index = index.wrapping_add(fast_prng(&mut prng_state) % 1001).wrapping_sub(500) % N;
                _ = black_box(v.get(index));
            }
        });
    });
    ...
    group.bench_function("SegVec/cached", |b| {
        ...
        b.iter_with_large_drop(|| {
            for _ in 0..N {
                index = index.wrapping_add(fast_prng(&mut prng_state) % 1001).wrapping_sub(500) % N;
                _ = black_box(v.get_cached(index, &mut cache));
            }
        });
    });
}
